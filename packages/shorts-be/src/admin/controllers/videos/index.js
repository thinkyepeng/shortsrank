...
const { LIST_FIELDS } = require('../../models/video');
...
async function getList(req, res, next) {
  ...
  if (keyword.length > 0) {
    ...
    } else if (searchType === 'playletId') {
      queryBuilder.andWhere('playlet_id', keyword);
      totalBuilder.andWhere('playlet_id', keyword);
    }
  }
...
async function updateData(req, res, next) {
  ...
  const {
    title, id, num, link, free, playlet_id,
  } = req.body;
  const updateData = {
    title, num, link, free, playlet_id,
  };
  if (id) {
    try {
...
async function deleteData(req, res, next) {
  const id = paramUtils.getInteger(req.body, 'id', '');
  if (id) {
    try {
